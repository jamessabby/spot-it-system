<?php
/**
 * S.P.O.T.-IT — Surrender Log
 * pages/surrender-log.php
 * MICROSERVICES: No SQL. All data from auth/ handlers via JS.
 */
require_once __DIR__ . '/../config/env.php';
$active_page = 'surrender_log';
...
<head>
  ...
  <title>Surrender Log — S.P.O.T.-IT</title>
...
<div class="app-shell">
  ...
  <div class="main-content">
    <div class="topbar">
      ...
      <div>
        <span class="topbar-title">Surrender Log</span>
      </div>
    ...
    <div class="page-body">
      ...
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:22px;">
        ...
        <div>
          <div style="font-family:var(--font-display);font-size:1.05rem;font-weight:800;color:var(--text-primary);">Surrender Log</div>
      ...
      <div class="card">
        <div class="card-head">
          <div class="card-title"><i class="fa-solid fa-inbox"></i> Surrender Log</div>
          <button class="btn btn-primary btn-sm" onclick="openModal('surrenderModal')">
            <i class="fa-solid fa-plus"></i> Record Surrender
          </button>
        </div>
        <table class="data-table" id="surrenderTable">
          <thead><tr><th>Date</th><th>Item Description</th><th>Surrendered By</th><th>Room / Location</th><th>Condition</th><th>Processed By</th><th>Status</th><th></th></tr></thead>
          <tbody id="surrenderBody">
            <tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--text-dim);font-size:.8rem;">Loading surrender records…</td></tr>
          </tbody>
        </table>
      </div>
      <div class="modal-overlay" id="surrenderModal" onclick="if(event.target===this)closeModal('surrenderModal')">
        <div class="modal-box" style="max-width:480px;">
          <div class="modal-head">
            <div class="modal-title">Record Item Surrender</div>
            <div class="modal-close" onclick="closeModal('surrenderModal')"><i class="fa-solid fa-xmark"></i></div>
          </div>
          <div class="modal-body">
            <div class="form-group"><label class="form-label">Item Description *</label><input type="text" class="form-control" id="sItemDesc" placeholder="e.g. Black umbrella with hook handle"/></div>
            <div class="form-group"><label class="form-label">Found Location *</label><input type="text" class="form-control" id="sLocation" placeholder="e.g. MLH 306 Workstation 5"/></div>
            <div class="form-group"><label class="form-label">Item Condition</label>
              <select class="form-control" id="sCondition"><option>Good</option><option>Damaged</option><option>Incomplete</option></select></div>
            <div class="form-group"><label class="form-label">Surrendered By (optional)</label><input type="text" class="form-control" id="sSurrenderedBy" placeholder="Name of person surrendering item"/></div>
            <div class="modal-actions">
              <button class="modal-btn dismiss" onclick="closeModal('surrenderModal')">Cancel</button>
              <button class="modal-btn recover" onclick="recordSurrender()"><i class="fa-solid fa-check"></i> Record</button>
            </div>
          </div>
        </div>
      </div>
